...
def load_issues(content_root: Path):
    ...
    for path in sorted((content_root / "issues").glob("*.json")):
        value = upgrade_legacy_issue(json.loads(path.read_text(encoding="utf-8")))
...
def load_issue_items(content_root: Path):
    ...
    for path in sorted((content_root / "issue-items").glob("*/*.json")):
        value = upgrade_legacy_issue_item(json.loads(path.read_text(encoding="utf-8")))
...
def commit_publication(content_root: Path, issue, issue_items, indexes) -> None:
    ...
    for item in issue_items:
        _write_json(
            staging / "issue-items" / published_date / f'{item["item_id"]}.json', item
        )
    ...
    swaps = []
    targets = (
        (staging / "issues" / f"{published_date}.json", content_root / "issues" / f"{published_date}.json"),
        (staging / "issue-items" / published_date, content_root / "issue-items" / published_date),
