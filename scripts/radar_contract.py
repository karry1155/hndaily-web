...
SCHEMA_VERSION = 13
PROMPT_VERSION = "hnhot-v4.3"
SOURCE_CANDIDATE_FIELDS = {
    "candidate_id", "item_id", "source", "title", "content", "original_url",
    "published_date", "collected_date", "page_number", "page_name",
    "page_url", "pdf_url", "page_sequence",
}
BLOCK_FIELDS = {"source", "title", "content", "ai_summary", "original_url"}
ITEM_ID_RE = re.compile(r"^hndaily-\d{8}-(?:\d+-\d+|url-[0-9a-f]{16})$")
...
def canonicalize_source_url(value: Any) -> str:
    ...
    return urlunsplit(("https", netloc, parsed.path.rstrip("/") or "/", query, ""))
