...
TOPIC_INPUT_DIR="$PRODUCTION_ROOT/topic-resolution-input"
TOPIC_OUTPUT_DIR="$PRODUCTION_ROOT/topic-resolution"
AUDIT_DIR="$PRODUCTION_ROOT/audit"
DATE_ARG="${1:-}"
CONTENT_ROOT="${RADAR_CONTENT_ROOT:-$WEB_DIR/content}"
SITE_ROOT="${RADAR_SITE_ROOT:-$WEB_DIR/site}"
mkdir -p "$RAW_DIR" "$MODEL_INPUT_DIR" "$MODEL_OUTPUT_DIR" "$TOPIC_INPUT_DIR" "$TOPIC_OUTPUT_DIR" "$AUDIT_DIR"
...
PROMPT_DIR="$WEB_DIR/prompts/article-enrichment/v3"
TOPIC_RESOLUTION_INPUT_JSON="$TOPIC_INPUT_DIR/$DATE_STEM.json"
TOPIC_RESOLUTION_OUTPUT_JSON="${RADAR_TOPIC_RESOLUTION_OUTPUT_JSON:-$TOPIC_OUTPUT_DIR/$DATE_STEM.json}"
TOPIC_PROMPT_DIR="$WEB_DIR/prompts/topic-resolution/v1"
...
printf 'RAW_JSON=%s\nMODEL_INPUT_JSON=%s\nMODEL_OUTPUT_JSON=%s\nPROMPT_DIR=%s\nTOPIC_RESOLUTION_INPUT_JSON=%s\nTOPIC_RESOLUTION_OUTPUT_JSON=%s\nTOPIC_PROMPT_DIR=%s\nPREFILTER_JSON=%s\nAUDIT_JSON=%s\n' "$RAW_JSON" "$MODEL_INPUT_JSON" "$MODEL_OUTPUT_JSON" "$PROMPT_DIR" "$TOPIC_RESOLUTION_INPUT_JSON" "$TOPIC_RESOLUTION_OUTPUT_JSON" "$TOPIC_PROMPT_DIR" "$PREFILTER_JSON" "$AUDIT_JSON"
...
if ! python3 "$WEB_DIR/scripts/prepare_topic_resolution.py" "$MODEL_OUTPUT_JSON" "$CONTENT_ROOT" "$TOPIC_RESOLUTION_INPUT_JSON" "$TOPIC_RESOLUTION_OUTPUT_JSON"; then
  echo "STATUS=TOPIC_RESOLUTION_REQUIRED"
  exit 3
fi
if ! python3 "$WEB_DIR/scripts/check_topic_resolution.py" "$TOPIC_RESOLUTION_INPUT_JSON" "$TOPIC_RESOLUTION_OUTPUT_JSON" "$CONTENT_ROOT"; then
  echo "STATUS=TOPIC_RESOLUTION_REQUIRED"
  exit 3
fi
python3 "$WEB_DIR/scripts/radar_transaction.py" prepare "$CONTENT_ROOT" "$STAGED_CONTENT"
python3 "$WEB_DIR/scripts/finalize_radar.py" "$RAW_JSON" "$MODEL_INPUT_JSON" "$MODEL_OUTPUT_JSON" "$TOPIC_RESOLUTION_INPUT_JSON" "$TOPIC_RESOLUTION_OUTPUT_JSON" "$STAGED_CONTENT" "$STAGED_AUDIT"
