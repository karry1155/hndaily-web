...
当前运行时契约为 `schema_version: 8` / `prompt_version: hnhot-v2.2`。v2.2 自由提取本篇与具体行动或结果直接相关的主体与事件，记录原文明确声明的主体简称或别称，不以第一次入库结果决定人物页准入，并支持书名号规划文件 `plans`；地点和主题仍只从受控候选中选择。发布流程保留每一篇通过确定性过滤的有效报道，不评分、不推荐、不二次精选。
